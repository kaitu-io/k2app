...
export default function TicketsPage() {
  ...
  // 默认 "false"：只看人工提交，避免自动汇报淹没视图。URL 显式传 "" 才看全部。
  const filterAutoGenerated = searchParams.has("autoGenerated")
  ...
  const [localAutoGenerated, setLocalAutoGenerated] = useState(filterAutoGenerated);
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      try {
        const response = await api.getFeedbackTickets({
          ...
          autoGenerated: filterAutoGenerated === "true" || filterAutoGenerated === "false"
            ? (filterAutoGenerated as "true" | "false")
            : undefined,
        });
    ...
    fetchTickets();
  }, [page, pageSize, filterStatus, filterEmail, filterUdid, filterAutoGenerated, refreshKey]);
  ...
  const handleFilter = () => {
    ...
    // 显式持久化 autoGenerated 选择（包括 ""=全部），覆盖默认 "false"
    params.set("autoGenerated", localAutoGenerated);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end gap-4 p-4 bg-muted/50 rounded-lg">
        ...
        <div className="flex-1">
          ...
          <select
            className="w-full p-2 border border-border bg-muted text-foreground rounded-md"
            value={localAutoGenerated}
            onChange={(e) => setLocalAutoGenerated(e.target.value)}
          >
            <option value="false">{"仅人工"}</option>
