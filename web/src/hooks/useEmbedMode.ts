...
export function useEmbedMode(): EmbedModeConfig {
  ...
  // External link interception lives in public/embed-interceptor.js (loaded
  // beforeInteractive from the [locale] layout) — NOT here. A React effect only
  // attaches after hydration, leaving early clicks to the native webview's
  // default new-window behaviour. Do not re-add a click handler here: it would
  // double-post 'external-link' messages and open links twice.
