...
title: k2s 服务端部署
date: 2026-02-22
summary: 在 Linux VPS 上一键安装并运行 k2s 服务端。零配置启动——自动生成密钥证书，k2arc 算法自动优化传输性能。
...
k2s 是 k2 协议的服务端程序，设计目标是**零配置启动**。您只需一条命令，服务端即可自动完成所有密钥生成和服务注册。k2arc 自适应速率控制算法在服务端同步启用，自动优化传输性能。
...
> k2s 同时自动启用 k2arc 拥塞控制。服务端和客户端协同运行 k2arc 算法，无需任何手动调优。
