...
title: k2 协议概述
date: 2026-02-22
summary: k2 是 Kaitu 自研隐身隧道协议族。当前版本 k2v5 搭载 k2arc 自适应速率控制算法，在高丢包、高审查网络中自动最大化吞吐量，无需手动配置带宽。
section: overview
order: 1
draft: false
---
...
k2v5 搭载自研的 **k2arc（Adaptive Rate Control）自适应速率控制算法**，能在高丢包、高延迟的网络环境中自动寻找最优发送速率——无需手动配置带宽参数。以 **QUIC/HTTP3** 作为主传输协议，并在 QUIC 被封锁时自动切换到 **TCP-WebSocket** 回退，配合 ECH 加密 SNI 和 TLS 指纹伪装，使隧道流量与真实 HTTPS 浏览流量无法区分。

## k2v5 三大核心能力

### k2arc 自适应速率控制

k2arc 是 k2v5 的核心竞争力。与传统拥塞控制算法不同，k2arc **自动探测**网络最优发送速率：

| 能力 | k2arc（k2v5） | 传统方案（如 Brutal） |
|------|----------|-------------------|
...
k2arc 的核心创新在于**审查感知机制**：在高审查网络中，大量丢包来自防火墙主动丢弃而非网络拥塞。k2arc 能自动区分审查丢包与拥塞丢包，避免错误地降低发送速率，从而在 GFW 等高丢包环境中维持远高于传统算法的吞吐量。

详细介绍见 [k2arc 自适应速率控制](/k2/protocol)。性能对比见 [k2 vs Hysteria2](/k2/vs-hysteria2)。
...
| [拥塞控制算法 k2arc](/k2/protocol) | k2arc 核心能力、审查感知、自动速率探测 |
