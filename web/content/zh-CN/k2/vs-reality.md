...
k2 和 VLESS+Reality 是当前两种最具代表性的隐身隧道方案，但它们采用了截然不同的技术路线。k2 通过 **ECH 加密 SNI + k2arc 自适应拥塞控制** 实现隐身与高性能的统一；VLESS+Reality 通过**借用真实网站的 TLS 证书**伪装流量身份。本文从 TLS 指纹、GFW 检测对抗、传输性能三个维度进行技术对比，所有分析基于公开学术研究和协议源码。
...
k2 搭载 [k2arc 自适应速率控制算法](/k2/protocol)，并以 QUIC/H3 作为主传输协议。
...
| 连接迁移 | QUIC 支持 | 不支持 |
| 审查性丢包处理 | 自适应 α 区分拥塞/审查丢包 | BBR 无法区分 |
...
- **k2arc**：GFW 模式 α=0.5 使算法几乎不因审查丢包降速，同时通过 RTT 感知避免 bufferbloat
...
在 GFW 概率性丢包（约 26%）环境下，BBR 的带宽估计会被持续丢包严重干扰，导致低估可用带宽。k2arc 的 GFW 模式（α=0.5）对审查丢包几乎不降速。详细的性能对比和 14 种测试场景请参阅 [k2 vs Hysteria2 拥塞控制对比](/k2/vs-hysteria2)。
