...
title: 隐身伪装技术
date: 2026-02-22
summary: k2 的隐身机制：ECH 加密 SNI、TLS 指纹伪装、流量特征隐藏、主动探测对抗，以及 k2arc 算法如何在隐身传输中维持高吞吐量。
section: technical
order: 6
draft: false
---
...
k2 的设计出发点是：即使在深度包检测（DPI）和主动探测普遍部署的网络环境中，连接流量也应与普通 HTTPS 浏览流量无法区分。同时，隐身不应以牺牲性能为代价——k2 的 [k2arc 算法](/k2/protocol) 确保在隐身传输中依然维持最优吞吐量。
...
隐身伪装解决的是"能不能连上"的问题，而 k2arc 算法解决的是"连上后跑多快"的问题。两者协同工作：

- **隐身层**确保连接不被识别和封锁
- **k2arc 算法**在隐身传输通道内自动最大化吞吐量，即使面对审查引发的高丢包也能维持高速

这意味着 k2 用户不需要在安全性和性能之间做取舍——ECH + TLS 伪装确保连接隐蔽，k2arc 确保连接高效。

---

如需了解 k2arc 算法的底层实现细节，请参阅 [协议技术详解](/k2/protocol)。如需了解 k2arc 与其他算法的性能对比，请参阅 [k2 vs Hysteria2](/k2/vs-hysteria2)。
