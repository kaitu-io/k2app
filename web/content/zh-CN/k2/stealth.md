...
k2 的设计出发点是：即使在深度包检测（DPI）和主动探测普遍部署的网络环境中，连接流量也应与普通 HTTPS 浏览流量无法区分。同时，隐身不应以牺牲性能为代价——[k2cc](/k2/k2cc) 确保在隐身传输中依然维持最优吞吐量。
...
- **隐身层**确保连接不被识别和封锁
- **k2cc**在隐身传输通道内自动最大化吞吐量，即使面对审查引发的高丢包也能维持高速
...
有效。GFW 向 k2s 发送不含 ECH 的 TLS 连接时，k2s 将连接透明转发到真实 CDN 服务器。GFW 收到的是合法的 HTTPS 响应——与直接访问该 CDN 完全一致。自动化探测脚本无法区分 k2s 与真实 CDN。VLESS+Reality 也有类似机制，但其 TLS Session ID 携带认证数据的特征已被学术研究识别（USENIX Security 2025）。

**TLS 指纹可以被检测出来吗？**

极难。k2 使用 uTLS 字节级精确复制真实浏览器的 ClientHello 特征。JA3/JA4 指纹分析工具无法将 k2 流量与真实 Chrome/Firefox/Safari 流量区分。这比 VLESS+Reality 的指纹方案更可靠——Reality 需要借用真实网站的证书，而 k2 使用自签名证书 + 证书固定，不依赖任何第三方。

**如果 GFW 封锁了 ECH 协议怎么办？**

直接封锁 ECH 会影响所有使用 ECH 的 CDN 服务（Cloudflare、Google 等），造成巨大附带损害，因此 GFW 目前的策略是干扰 ECH 密钥的 DNS 分发。k2 的应对是：URL 中的 `ech=...` 参数直接包含 ECH 密钥，完全不依赖 DNS 获取密钥，绕过了 GFW 的 DNS 干扰。

**自签名证书安全吗？**

完全安全。k2 使用证书固定（`pin=sha256:HASH`），客户端直接验证服务端证书的 SHA-256 哈希，不依赖 CA 信任链。自签名证书不进入 Certificate Transparency 日志，避免被 CT 扫描发现——这是对 CA 签发证书的隐蔽性优势。
