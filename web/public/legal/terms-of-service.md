# 服务条款 / Terms of Service

**最后更新 / Last Updated: 2026-07**

---

## 中文版本

### 一、协议接受与生效

1.1 本服务条款（以下简称「本协议」）是您与开途（Kaitu）之间关于使用开途网络代理服务（以下简称「本服务」）的法律协议。

1.2 通过注册账户、下载安装客户端、访问或使用本服务，即表示您已阅读、理解并同意受本协议所有条款的约束。如果您不同意本协议的任何条款，请勿使用本服务。

1.3 如果您代表公司或其他法律实体使用本服务，您声明并保证您有权代表该实体接受本协议，且该实体同意对您违反本协议的行为承担责任。

1.4 本协议自您首次使用本服务之日起生效，除非根据本协议规定终止，否则持续有效。

### 二、服务说明

2.1 **服务内容**：开途提供网络代理服务，通过加密隧道技术帮助用户安全地访问互联网。服务包括但不限于：VPN 连接、流量加密、IP 地址保护等功能。

2.2 **服务计划**：本服务以订阅制提供，包括不同时长和功能的套餐选项。具体服务内容和价格以购买时页面显示为准。

2.3 **服务可用性**：我们致力于提供稳定可靠的服务，但不保证服务 100% 无中断。我们可能因系统维护、升级或不可抗力等原因暂时中断服务，并会尽量提前通知用户。

2.4 **服务变更**：我们保留在合理通知期内修改、更新或终止部分服务功能的权利。重大变更将提前通过电子邮件或应用内通知告知用户。

### 三、用户义务与责任

3.1 **账户安全**：您有责任保管好您的账户凭据，不得将账户借给他人使用。因账户凭据泄露导致的任何损失由您自行承担。

3.2 **合法使用**：您同意仅将本服务用于合法目的，遵守您所在地区以及服务器所在地区的所有适用法律法规。

3.3 **准确信息**：您同意在注册和使用过程中提供真实、准确、完整的信息，并及时更新以保持其准确性。

3.4 **设备要求**：您有责任确保您的设备满足使用本服务的最低系统要求，并承担因设备问题导致无法正常使用服务的风险。

3.5 **第三方内容**：您通过本服务访问的任何第三方网站或服务的内容、隐私政策和做法均与开途无关，您需自行承担相关风险。

### 四、禁止行为

4.1 您不得将本服务用于以下目的：

(a) 从事任何违反适用法律法规的活动；
(b) 传输或存储任何侵犯他人知识产权、隐私权或其他合法权益的内容；
(c) 传播病毒、恶意软件、垃圾邮件或其他有害内容；
(d) 进行网络攻击、黑客行为或试图未经授权访问他人系统；
(e) 进行任何可能损害、禁用、过载或损伤我们服务器或网络的行为；
(f) 干扰其他用户正常使用本服务；
(g) 试图绕过本服务的任何安全功能或访问限制；
(h) 转售、出租或以任何方式商业化使用本服务，除非获得我们的书面授权。

4.2 违反上述规定可能导致您的账户被暂停或终止，且不予退款。严重违规行为可能被报告给相关执法部门。

4.3 **公平使用**：本服务的网络资源由全体用户共享。若您的流量使用量持续显著高于正常水平并影响其他用户的服务体验，我们可能先行提醒，并对您的账号采取限速措施；经提醒仍未改善的，我们保留依第 4.2 条暂停或终止服务的权利。如您有持续的大流量需求，可联系我们了解专属线路方案。

### 五、服务可用性与免责

5.1 **服务状态**：本服务按「现状」和「可用」的基础提供。我们不保证服务将不间断、及时、安全或无错误。

5.2 **网络条件**：服务质量可能受您的互联网连接、网络拥塞和其他我们无法控制的因素影响。

5.3 **第三方限制**：某些国家或地区可能限制或禁止使用 VPN 服务。您有责任了解并遵守您所在地区的相关法规。

5.4 **计划维护**：我们会定期进行系统维护和升级，通常会提前通知用户。紧急维护可能不提前通知。

5.5 **不可抗力**：因自然灾害、战争、政府行为、网络攻击或其他不可抗力事件导致的服务中断，我们不承担责任。

### 六、付款与计费

6.1 **费用标准**：服务费用按您选择的订阅计划收取，具体价格以购买时显示为准。价格可能因促销活动或地区而有所不同。

6.2 **支付方式**：我们接受多种支付方式，所有支付均通过安全的第三方支付处理商处理。

6.3 **自动续费**：除非您在当前订阅期结束前取消，订阅将自动续费。续费价格可能与首次购买价格不同。

6.4 **价格变更**：我们保留调整服务价格的权利。价格变更将在您下一个订阅周期生效，我们会提前通知您。

6.5 **税费**：显示价格可能不含适用的税费。您有责任支付所有与购买相关的税费。

### 七、退款政策

7.1 **退款条件**：如果您对服务不满意，可在首次购买后 7 天内申请退款。退款申请需满足以下条件：
- (a) 首次购买该类型套餐；
- (b) 账户使用流量未超过 1GB；
- (c) 账户状态正常，未因违规被暂停。

7.2 **退款方式**：批准的退款将以钱包余额形式退回您的账户。钱包余额可用于：
- (a) 购买或续费任何套餐服务；
- (b) 申请提现到数字货币钱包。

7.3 **退款处理时间**：退款申请将在 3-5 个工作日内审核处理。退款到钱包后立即可用。

7.4 **不予退款情形**：以下情况不适用退款政策：
- (a) 超出 7 天退款期限；
- (b) 使用流量超过 1GB；
- (c) 账户因违规被暂停或终止；
- (d) 促销套餐或已明确标注不可退款的产品。

### 八、钱包与提现

8.1 **钱包余额来源**：钱包余额可能来自退款、推广奖励或其他活动奖励。

8.2 **提现方式**：我们支持将钱包余额提现到数字货币钱包，支持的币种包括：
- (a) USDT（TRC20/ERC20 网络）
- (b) USDC（ERC20 网络）
- (c) 其他主流数字货币（以实际支持为准）

8.3 **提现手续费**：提现可能产生手续费，具体费用在提现申请时显示。

8.4 **处理时间**：提现申请将在 1-3 个工作日内处理。区块链确认时间取决于网络状况。

8.5 **地址责任**：请务必提供正确的钱包地址和区块链网络。因地址错误导致的资金损失由用户自行承担，我们无法追回。

8.6 **最低提现金额**：提现有最低金额限制，具体以提现页面显示为准。

### 九、协议终止

9.1 **用户终止**：您可以随时通过删除账户来终止本协议。账户删除后，您的订阅将立即终止，不予退款。

9.2 **我方终止**：我们保留在以下情况下暂停或终止您的账户的权利：
- (a) 您违反本协议的任何条款；
- (b) 您的账户长期处于非活跃状态（超过 12 个月）；
- (c) 因法律要求或配合执法调查；
- (d) 我们决定停止提供服务。

9.3 **终止后果**：协议终止后：
- (a) 您对服务的访问权限将立即停止；
- (b) 您的账户数据将按隐私政策处理；
- (c) 未使用的订阅时间不予退款，除非法律另有规定；
- (d) 本协议中关于责任限制、免责声明和争议解决的条款继续有效。

### 十、责任限制

10.1 **服务免责**：在法律允许的最大范围内，我们不对以下情况承担责任：
- (a) 服务中断或不可用造成的损失；
- (b) 数据丢失或损坏；
- (c) 您通过本服务访问的第三方内容；
- (d) 任何间接、附带、特殊、惩罚性或后果性损害。

10.2 **责任上限**：在任何情况下，我们的总责任不超过您在索赔前 12 个月内支付给我们的服务费用总额。

10.3 **第三方行为**：我们不对第三方的行为负责，包括但不限于：
- (a) 您访问的网站或服务；
- (b) 支付处理商的行为；
- (c) 网络服务提供商的行为。

10.4 **不可预见损失**：我们不对任何我们无法合理预见的损失承担责任。

### 十一、条款变更

11.1 **修改权利**：我们保留随时修改本协议的权利。修改后的条款将在发布后生效。

11.2 **变更通知**：重大条款变更将通过以下方式通知：
- (a) 在网站或应用显著位置发布公告；
- (b) 通过电子邮件通知注册用户；
- (c) 应用内推送通知。

11.3 **继续使用**：在条款变更通知后继续使用本服务，即表示您接受修改后的条款。如果您不同意修改后的条款，应立即停止使用本服务。

11.4 **历史版本**：您可以联系我们获取本协议的历史版本。

### 十二、联系方式与争议解决

12.1 **联系方式**：如果您对本协议有任何疑问或投诉，请通过以下方式联系我们：

**电子邮件**：legal@kaitu.io

12.2 **争议解决**：因本协议产生的或与本协议相关的任何争议，双方应首先通过友好协商解决。

12.3 **适用法律**：本协议的解释和执行适用相关法律法规。

12.4 **可分割性**：如果本协议的任何条款被认定为无效或不可执行，该条款应在最小必要范围内予以修改，其余条款继续有效。

12.5 **完整协议**：本协议（包括隐私政策和其他引用的政策）构成您与我们之间关于本服务的完整协议，取代之前的所有口头或书面协议。

---

## English Version

### 1. Agreement Acceptance and Effectiveness

1.1 These Terms of Service (hereinafter referred to as "this Agreement") constitute a legal agreement between you and Kaitu regarding the use of Kaitu network proxy services (hereinafter referred to as "the Service").

1.2 By registering an account, downloading and installing the client, accessing, or using the Service, you acknowledge that you have read, understood, and agree to be bound by all terms of this Agreement. If you do not agree to any terms of this Agreement, please do not use the Service.

1.3 If you are using the Service on behalf of a company or other legal entity, you represent and warrant that you have the authority to accept this Agreement on behalf of such entity, and that entity agrees to be responsible for your violation of this Agreement.

1.4 This Agreement takes effect from the date you first use the Service and remains in effect unless terminated in accordance with this Agreement.

### 2. Service Description

2.1 **Service Content**: Kaitu provides network proxy services that help users securely access the internet through encrypted tunnel technology. Services include but are not limited to: VPN connections, traffic encryption, IP address protection, and other features.

2.2 **Service Plans**: The Service is provided on a subscription basis, including package options of different durations and features. Specific service content and pricing are subject to the display at the time of purchase.

2.3 **Service Availability**: We are committed to providing stable and reliable service but do not guarantee 100% uninterrupted service. We may temporarily interrupt service due to system maintenance, upgrades, or force majeure, and will try to notify users in advance.

2.4 **Service Changes**: We reserve the right to modify, update, or terminate certain service features with reasonable notice. Significant changes will be communicated to users in advance via email or in-app notification.

### 3. User Obligations and Responsibilities

3.1 **Account Security**: You are responsible for safeguarding your account credentials and must not lend your account to others. Any losses resulting from credential disclosure are your sole responsibility.

3.2 **Legal Use**: You agree to use the Service only for lawful purposes and to comply with all applicable laws and regulations in your region and the server locations.

3.3 **Accurate Information**: You agree to provide true, accurate, and complete information during registration and use, and to update it promptly to maintain accuracy.

3.4 **Device Requirements**: You are responsible for ensuring your device meets the minimum system requirements for using the Service and bear the risk of being unable to use the service properly due to device issues.

3.5 **Third-Party Content**: The content, privacy policies, and practices of any third-party websites or services you access through the Service are unrelated to Kaitu, and you assume all associated risks.

### 4. Prohibited Activities

4.1 You may not use the Service for the following purposes:

(a) Engaging in any activities that violate applicable laws and regulations;
(b) Transmitting or storing any content that infringes on others' intellectual property rights, privacy rights, or other legal rights;
(c) Spreading viruses, malware, spam, or other harmful content;
(d) Conducting cyberattacks, hacking, or attempting unauthorized access to others' systems;
(e) Engaging in any activities that may damage, disable, overload, or impair our servers or network;
(f) Interfering with other users' normal use of the Service;
(g) Attempting to bypass any security features or access restrictions of the Service;
(h) Reselling, renting, or commercially using the Service in any way without our written authorization.

4.2 Violation of the above provisions may result in suspension or termination of your account without refund. Serious violations may be reported to relevant law enforcement authorities.

4.3 **Fair Use**: The Service's network resources are shared by all users. If your traffic usage remains significantly and persistently above normal levels and affects other users' experience, we may notify you and apply speed limits to your account; if the situation does not improve after notice, we reserve the right to suspend or terminate the Service under Section 4.2. If you have sustained high-traffic needs, please contact us to learn about dedicated line options.

### 5. Service Availability and Disclaimers

5.1 **Service Status**: The Service is provided on an "as is" and "as available" basis. We do not guarantee that the Service will be uninterrupted, timely, secure, or error-free.

5.2 **Network Conditions**: Service quality may be affected by your internet connection, network congestion, and other factors beyond our control.

5.3 **Third-Party Restrictions**: Some countries or regions may restrict or prohibit the use of VPN services. You are responsible for understanding and complying with relevant regulations in your region.

5.4 **Scheduled Maintenance**: We regularly perform system maintenance and upgrades, typically with advance notice to users. Emergency maintenance may occur without advance notice.

5.5 **Force Majeure**: We are not liable for service interruptions caused by natural disasters, war, government actions, cyberattacks, or other force majeure events.

### 6. Payment and Billing

6.1 **Fee Standards**: Service fees are charged according to your chosen subscription plan, with specific pricing subject to the display at the time of purchase. Prices may vary due to promotional activities or region.

6.2 **Payment Methods**: We accept multiple payment methods, with all payments processed through secure third-party payment processors.

6.3 **Auto-Renewal**: Unless you cancel before the end of the current subscription period, subscriptions will auto-renew. Renewal prices may differ from the initial purchase price.

6.4 **Price Changes**: We reserve the right to adjust service prices. Price changes will take effect in your next subscription cycle, and we will notify you in advance.

6.5 **Taxes**: Displayed prices may not include applicable taxes. You are responsible for paying all taxes associated with your purchase.

### 7. Refund Policy

7.1 **Refund Conditions**: If you are not satisfied with the Service, you may request a refund within 7 days of initial purchase. Refund requests must meet the following conditions:
- (a) First-time purchase of that package type;
- (b) Account data usage has not exceeded 1GB;
- (c) Account is in good standing and has not been suspended for violations.

7.2 **Refund Method**: Approved refunds will be returned to your account as wallet balance. Wallet balance can be used to:
- (a) Purchase or renew any package service;
- (b) Request withdrawal to a cryptocurrency wallet.

7.3 **Refund Processing Time**: Refund requests will be reviewed and processed within 3-5 business days. Refunds are immediately available once credited to your wallet.

7.4 **Non-Refundable Cases**: The refund policy does not apply to:
- (a) Requests exceeding the 7-day refund period;
- (b) Data usage exceeding 1GB;
- (c) Accounts suspended or terminated for violations;
- (d) Promotional packages or products explicitly marked as non-refundable.

### 8. Wallet and Withdrawals

8.1 **Wallet Balance Sources**: Wallet balance may come from refunds, promotional rewards, or other activity rewards.

8.2 **Withdrawal Methods**: We support withdrawing wallet balance to cryptocurrency wallets, with supported currencies including:
- (a) USDT (TRC20/ERC20 networks)
- (b) USDC (ERC20 network)
- (c) Other mainstream cryptocurrencies (subject to actual support)

8.3 **Withdrawal Fees**: Withdrawals may incur fees, with specific fees displayed at the time of withdrawal request.

8.4 **Processing Time**: Withdrawal requests will be processed within 1-3 business days. Blockchain confirmation time depends on network conditions.

8.5 **Address Responsibility**: Please ensure you provide the correct wallet address and blockchain network. Losses due to incorrect addresses are the user's sole responsibility and cannot be recovered.

8.6 **Minimum Withdrawal Amount**: Withdrawals have a minimum amount limit, subject to the display on the withdrawal page.

### 9. Agreement Termination

9.1 **User Termination**: You may terminate this Agreement at any time by deleting your account. After account deletion, your subscription will immediately terminate without refund.

9.2 **Our Termination**: We reserve the right to suspend or terminate your account under the following circumstances:
- (a) You violate any terms of this Agreement;
- (b) Your account has been inactive for an extended period (over 12 months);
- (c) As required by law or to cooperate with law enforcement investigations;
- (d) We decide to discontinue providing the Service.

9.3 **Termination Consequences**: Upon Agreement termination:
- (a) Your access to the Service will immediately cease;
- (b) Your account data will be handled according to the Privacy Policy;
- (c) Unused subscription time will not be refunded unless otherwise required by law;
- (d) Provisions of this Agreement regarding liability limitations, disclaimers, and dispute resolution remain in effect.

### 10. Limitation of Liability

10.1 **Service Disclaimers**: To the maximum extent permitted by law, we are not liable for:
- (a) Losses caused by service interruptions or unavailability;
- (b) Data loss or damage;
- (c) Third-party content you access through the Service;
- (d) Any indirect, incidental, special, punitive, or consequential damages.

10.2 **Liability Cap**: In no event shall our total liability exceed the total service fees you paid to us in the 12 months preceding the claim.

10.3 **Third-Party Actions**: We are not responsible for the actions of third parties, including but not limited to:
- (a) Websites or services you access;
- (b) Payment processor actions;
- (c) Network service provider actions.

10.4 **Unforeseeable Losses**: We are not liable for any losses that we could not reasonably foresee.

### 11. Terms Changes

11.1 **Right to Modify**: We reserve the right to modify this Agreement at any time. Modified terms will take effect upon posting.

11.2 **Change Notification**: Significant term changes will be notified through:
- (a) Posting announcements in prominent locations on the website or app;
- (b) Email notification to registered users;
- (c) In-app push notifications.

11.3 **Continued Use**: Continuing to use the Service after notification of term changes indicates your acceptance of the modified terms. If you do not agree to the modified terms, you should immediately stop using the Service.

11.4 **Historical Versions**: You may contact us to obtain historical versions of this Agreement.

### 12. Contact Information and Dispute Resolution

12.1 **Contact Information**: If you have any questions or complaints about this Agreement, please contact us:

**Email**: legal@kaitu.io

12.2 **Dispute Resolution**: Any disputes arising from or related to this Agreement should first be resolved through friendly negotiation between the parties.

12.3 **Applicable Law**: The interpretation and enforcement of this Agreement shall be governed by applicable laws and regulations.

12.4 **Severability**: If any provision of this Agreement is found to be invalid or unenforceable, that provision shall be modified to the minimum extent necessary, and the remaining provisions shall continue in effect.

12.5 **Entire Agreement**: This Agreement (including the Privacy Policy and other referenced policies) constitutes the entire agreement between you and us regarding the Service, superseding all prior oral or written agreements.
