...
describe('cross-layer contract: brand registry (webapp ⇄ api/brand.go)', () => {
  ...
  describe.each(brandIds)('brand: %s', (id) => {
    ...
    it('every websiteOrigin hostname is owned by this brand on the backend', () => {
      for (const origin of webapp.websiteOrigins) {
        const hostname = new URL(origin).hostname;
        expect(
          backend.hosts,
          `${id}: websiteOrigin "${origin}" resolves to host "${hostname}", which the backend ` +
            `does not attribute to this brand (${backend.hosts.join(', ')}) — iframe postMessage ` +
            `would trust an origin the backend treats as another brand`
        ).toContain(hostname);
      }
    });
