 ...
 *  - 商品行（单一自动续订订阅），显示 StoreKit 本地化 displayName + displayPrice
 *  - 主购买按钮
...
/** The single auto-renewable subscription product id (parity with Center `1y`). */
...
export default function IapPurchaseSheet({
  ...
  // Single product row: prefer Apple-authoritative StoreKit metadata
  // (localized displayName + displayPrice), else a placeholder fallback for
  // load-failure / web. Never hardcode the price — StoreKit is the source.
  const row = useMemo(() => {
    const meta =
      products.find((p) => p.id === DEFAULT_SELECTED) ?? products[0];
    return {
