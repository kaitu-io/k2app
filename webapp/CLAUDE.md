...
**⛔ Relay 已由 kill-switch 关停（2026-07-17，#3288/#3289）**：`services/relay-flag.ts` 的 `RELAY_ENABLED = false` 短路了 relay-fetch 传输、relay-add-nodes 喂节点与 seed CDN 拉取——当前传输为**直连 only**（`antiblock.ts resolveEntry()` → `fetch`，14s 预算），并一次性清除 relay 时代污染的 `k2_entry_url` 缓存。relay 代码全保留，恢复 = 改一行重发版；详见 `docs/superpowers/specs/2026-07-17-disable-relay-restore-direct-design.md`。以下段落描述的是 `RELAY_ENABLED = true` 时的行为：
...
App is the sole management UI for a headless k2r router — the old embedded-webapp panel (`http://lanIP:1779` in a browser) is retired. Discovery and control both go through a **single anchor mechanism**: a constant `http://10.17.79.1:1779` (`ROUTER_ANCHOR` in `services/router-service.ts`) that k2r DNAT-intercepts on its forwarding path (any layer of a multi-hop gateway chain). There is no gateway-probe chain, no beacon-discovery consumption, and no `lanIP` concept in the app. Full design: `docs/superpowers/specs/2026-07-17-k2r-headless-app-control-design.md`.

- **`router-service.ts`**: `probeRouter()` — GET `{ROUTER_ANCHOR}/ping`, 1.5s timeout, validates `k2r: true` signature (rejects a real LAN device that happens to squat the anchor IP). `getControlKey(forceRefresh?)` — local-storage-cached, refetches from `POST /api/user/router-control-key` on miss or forced refresh. `routerFetch()` (shared by `routerCore()` and `routerDevicesGet/Post()`) sends `Authorization: Bearer <controlKey>`, and on HTTP 401 refetches the key once (`getControlKey(true)`) and retries — covers Center-side key rotation. All requests go through `_platform.routerRequest` (bridge boundary — see below), never raw `fetch`, except the `make dev-standalone` browser fallback.
- **`router.store.ts`** (zustand): four-phase state machine — `none` (never seen) / `unconfigured` (anchor answers but `configured:false`) / `online` / `offline` (was configured, anchor now unreachable — tab stays visible, greyed out, to avoid flicker). `startPolling()` runs `routerCore('status')` every 2s **only while the Router tab is mounted** (no SSE — CapacitorHttp doesn't support streaming). A late in-flight poll response is dropped if `unbindRouter()` completed in the meantime (checks `get().router` again before writing state) to prevent resurrecting a cleared router. `isRouterTakeover(state)` — the shared predicate for the Dashboard banner and the exclusion dialog: `phase === 'online' && status?.state === 'connected'`. Anchor reachability alone proves k2r sits on this device's forwarding path (spec §3.4), so no separate gateway check is needed.
- **`RouterExclusionDialog.tsx`** (`useExclusionGuard`): fires the MUI dual-connection warning **only at the moment a second connection is about to be established** — Router-tab connect-tap while local VPN is already connected, or Dashboard connect-tap while `isRouterTakeover()` is true. It does not fire on tab switches (a user who has a router but no active plan browsing the Router tab shouldn't be interrupted). Same component also owns the unbind confirmation dialog (MUI, not `window.confirm` — Capacitor WebView silently swallows it, constitutional rule).
- **`RouterTakeoverBanner.tsx`**: shown on Dashboard when `isRouterTakeover()` is true and local VPN state isn't `connected` — replaces a bare "disconnected" reading that would otherwise misread as unprotected while the router has taken over.
- **`RouterPage.tsx`**: top-level tab, sibling to Dashboard, added to `Layout.tsx`'s tab list only once `router.store.phase !== 'none'` (persists once seen, doesn't disappear on transient offline). Three sections: connection card, LAN device list (migrated `RouterDevices`, now sourced via `routerHttp` instead of same-origin fetch), settings (version, unbind).
- **Retired**: the embedded `platformType === 'gateway'` webapp-in-router-browser mode (`gateway-k2.ts`, `gateway-core.ts`, `gateway-storage.ts`, `GatewaySetup.tsx` all deleted in `521d346d`). The `'gateway'` `platformType` enum value is kept only because old k2r units in the field still run frozen pre-retirement webapp snapshots that set it at runtime — no code path in the current bundle can produce it. **Known follow-up, not yet done**: `SmartServerSelector`'s `k2sub` tab and Dashboard's `k2subTabContent`/`serverMode==='k2sub'` branches are still gated on `platformType==='gateway'` and are therefore dead in the current bundle — flagged for a later cleanup pass, out of scope for this feature.
- **Bridge boundary**: all LAN traffic to the router goes through `_platform.routerRequest` only — never a raw `fetch()` to a LAN IP from page/component code. Tauri backs it with a Rust `router_http_request` command (SSRF-gated); Capacitor backs it with `CapacitorHttp` (TS-side SSRF gate, since CapacitorHttp has no native allowlist). See `desktop/CLAUDE.md` and `mobile/CLAUDE.md` for the two implementations. `_platform.getDefaultGateway()` exists on both bridges but is an **unconsumed** optional capability — `router-service.ts` never calls it (discovery is anchor-only, per spec §5.1).
