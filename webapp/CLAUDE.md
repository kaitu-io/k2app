...
│   ├── services/           # cloudApi, k2api, authService, cacheStore, web-platform, standalone-k2, beta-auto-upload, antiblock, stats, api-types, vpn-types, device-udid, secure-storage, tauri-storage, capacitor-storage, plain-storage, subs-resolver (mobile-only k2subs:// → k2v5:// resolver; desktop daemon does this server-side)
...
- **k2subs resolver (mobile-only)**: `services/subs-resolver.ts` fetches `/api/subs` + picks a k2v5 URL via `pickWeighted` (recommendScore first, legacy weight fallback — mirrors `k2/config/subscription.go Pick()`). MUST be gated on `window._platform.platformType === 'mobile'` — desktop passes raw `k2subs://` to daemon which resolves server-side with refresh loop + Phase-B hot-swap. Calling the resolver on desktop is harmless but defeats auto-refresh/auto-switch. `connection.store.ts` already does the gating; any new caller must too. Failure on mobile path → engine code 570; see `mobile/CLAUDE.md` "Smart Server Selection".
