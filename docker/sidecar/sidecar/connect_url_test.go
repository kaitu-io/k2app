...
func TestBuildServerURL_Full(t *testing.T) {
	raw := "k2v5://udid:token@hk1.example.com:443?ech=AABBCCDD&pin=sha256:abc123&insecure=1&hop=10020-10119"
	url := BuildServerURL(raw, "hk1.example.com", 443, "", "")
...
func TestBuildServerURL_NoECH(t *testing.T) {
	...
	url := BuildServerURL(raw, "node.example.com", 443, "", "")
...
func TestBuildServerURL_OverridesDomainPort(t *testing.T) {
	...
	url := BuildServerURL(raw, "configured.com", 443, "", "")
...
func TestBuildServerURL_InvalidURL(t *testing.T) {
	url := BuildServerURL("not a valid url %%%", "test.com", 443, "", "")
	assert.Equal(t, "", url)
}

func TestBuildServerURL_EmptyString(t *testing.T) {
	url := BuildServerURL("", "test.com", 443, "", "")
	assert.Equal(t, "", url)
}

func TestBuildServerURL_NoParams(t *testing.T) {
	raw := "k2v5://udid:token@test.com:443"
	url := BuildServerURL(raw, "test.com", 443, "", "")
	assert.Equal(t, "", url, "should return empty when no ech or pin params")
}

func TestBuildServerURL_WithIPv4(t *testing.T) {
	raw := "k2v5://udid:token@hk1.example.com:443?ech=AABB&pin=sha256:abc&hop=10020-10119"
	url := BuildServerURL(raw, "hk1.example.com", 443, "1.2.3.4", "")
	assert.Contains(t, url, "hop=10020-10119")
...
func TestBuildServerURL_WithIPv4AndIPv6(t *testing.T) {
	raw := "k2v5://udid:token@hk1.example.com:443?ech=AABB&pin=sha256:abc&hop=10020-10119"
	url := BuildServerURL(raw, "hk1.example.com", 443, "1.2.3.4", "2001:db8::1")
	assert.Contains(t, url, "hop=10020-10119")
	assert.Contains(t, url, "ip=1.2.3.4")
...
func TestBuildServerURL_WithIPv6Only(t *testing.T) {
	...
	url := BuildServerURL(raw, "hk1.example.com", 443, "", "2001:db8::1")
