...
func newTestEnforcer(t *testing.T, m nicMeter, d dockerController) *enforcer {
	t.Helper()
	return newEnforcer(m, d, filepath.Join(t.TempDir(), "cutoff.state"), []string{"k2v5", "k2v4-slave"}, time.Second)
}

func TestEnforcer_CutsAtQuota(t *testing.T) {
	m := &fakeMeter{}
	d := newFakeDocker("k2v5", "k2v4-slave")
	e := newTestEnforcer(t, m, d)
	e.SetQuota(1, 1000, 0)
	m.set(999)
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "999<1000 不掐")
	m.set(1000)
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"), "到 1000 掐")
	assert.True(t, d.isPaused("k2v4-slave"), "所有数据面容器都掐")
}

func TestEnforcer_AnchorsToCenterUsed(t *testing.T) {
	m := &fakeMeter{}
	d := newFakeDocker("k2v5")
	e := newTestEnforcer(t, m, d)
	m.set(0)
	e.SetQuota(1, 1000, 950) // SetQuota 内部快照 meter=0
	m.set(51)
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"), "950+51>=1000 掐")
}
...
func TestEnforcer_UncutsOnNewEpoch(t *testing.T) {
	...
	m.set(1000)
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"))
	m.set(0)
	e.SetQuota(2, 1000, 0)
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "新周期自动恢复")
}
...
func TestEnforcer_NoQuotaIsNoop(t *testing.T) {
	...
	m.set(999999)
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "无配额不掐")
...
func TestEnforcer_RestartReappliesPersistedCut(t *testing.T) {
	...
	m := &fakeMeter{}
	d := newFakeDocker("k2v5")
	e := newEnforcer(m, d, path, []string{"k2v5"}, time.Second)
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"), "重启读 state,无配额也保持掐断")
}
...
func TestEnforcer_SelfHealsResurrectedContainer(t *testing.T) {
	...
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"))
	d.setPaused("k2v5", false) // docker 守护重启复活
	e.reconcile(context.Background())
	assert.True(t, d.isPaused("k2v5"), "下一 tick 自愈重掐")
}
...
func TestEnforcer_HoldsOnStaleAnchorNoFalseCut(t *testing.T) {
	...
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "50% 不掐")
	m.set(0) // epoch 翻转:reporter 已 Rebaseline,SetQuota 尚未到
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "陈旧 anchor 不得误掐干净线")
	e.SetQuota(2, 1000, 0) // 新 epoch 配额到达
	e.reconcile(context.Background())
	assert.False(t, d.isPaused("k2v5"), "新周期仍不掐")
}
