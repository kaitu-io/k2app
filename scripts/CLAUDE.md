...
| Verification | `test_build.sh` (count computed at runtime), `check-embed-size.sh` (committed `k2/rule/embed/krs.tar.gz` ≤ 300 KB, reads the k2 HEAD blob), `check-install-script-sync.sh` (`web/public/i/k2r` — what `kaitu.io/i/k2r` serves — must be byte-identical to `k2/scripts/install-k2r.sh`; hand-synced, so a k2 fix never reaches the served copy otherwise), `check-i18n.mjs` (webapp `yarn build` runs it `--ci`: locale keys vs zh-CN), `test-openwrt.sh` + `openwrt/` (`install.sh`, `k2r.init`, `luci-app-k2r`) |
...
- `ci/release-plan.mjs` (+ `.test.mjs`; `ci.yml` `test-ci-scripts`) — ref suffix → brand × platform legs + `tag_version`, the **only** copy of the release-tag table; `release-desktop.yml` and `build-mobile.yml` `plan` jobs both call it. The test pins every suffix, the "a brand scope covers the same platforms everywhere" rule, rule shadowing, and fails if either workflow re-grows a bash suffix table / `TAG_VERSION%-` strip list.
...
- `ci/web-ota-plan.mjs` (+ `.test.mjs`; `publish-web-ota.yml` `plan` job) — `webapp/x.y.z[-brand]` tag (or the dispatch `brand` input) → the brand set (bare=both, `-kaitu`/`-overleap`=one) + suffix-stripped `tag_version`. Separate from `release-plan.mjs` on purpose: no platform dimension, different `webapp/` prefix. Since 2026-09-18 web OTA is dedicated-tag-only; a `v*` app tag no longer publishes it.
...
- Windows signing chain: `desktop/src-tauri/windows-sign.sh` (Tauri `signCommand`, journals to `$K2_SIGN_LOG` incl. a `brand=` line) dispatches to `ci/macos/windows-sign.sh` (osslsigncode + SimplySign PKCS#11 + `-ac certum-chain.pem`) or `ci/windows/sign-binary.ps1` (signtool on a Windows host). The Authenticode description follows `K2_BRAND`: `ci/sign-description.sh` prints `Overleap Desktop` / `Kaitu Desktop` for osslsigncode `-n`, and `sign-binary.ps1` forks signtool `/d` (`Overleap` / `Kaitu`) on the same env; `test-sign-description.sh` (manual, no caller) pins all three brand values plus the ps1 arm. That chain is kept alive by `ci/windows-sign-preflight.sh` (signs a throwaway k2.exe before the 15-min bundle) plus the SimplySign cloud-session keepalive — which is **machine-level, not in this repo**: one agent, one cloud session, one Keychain entry per machine, shared by every project that signs Windows. Source of truth is `~/projects/wordgate/simplisign`; its installer deploys the runtime copy to `~/Library/Application Support/kaitu-simplisign/`, and both `make simplisign-login` and the release workflow call **that** copy — never a repo checkout, which CI wipes each build. Don't re-add a copy here: k2app's old one lacked the osascript-timeout and screen-lock gates, and running its installer would have silently reverted them.
...
- S3 → notify Lambda for log uploads (`s3-log-notify`, ap-northeast-1) — **source is not in this repo** (no `scripts/lambda/` exists; earlier docs pointed at a ghost path). Two independent gates, both on the AWS side: the **bucket notification rules** on `kaitu-service-logs` decide whether the Lambda fires at all, and the Lambda's own `PLATFORM_PREFIXES` decides whether it parses the key. Both now cover `desktop/`, `desktop-overleap/`, `mobile/` plus legacy `service-logs/` / `feedback-logs/` (2026-09-08: added the `desktop-overleap-logs-notify` rule + the set entry). The background rotated-log prefixes (`auto/` for kaitu, `auto-overleap/` for overleap — `S3_PREFIX_AUTO` in `desktop/src-tauri/src/log_upload.rs`) are **deliberately not notified**: high volume, and `auto/` has never had a rule. Mobile is not brand-split — both brands upload under `mobile/` (`K2Plugin.kt` / `.swift`).
