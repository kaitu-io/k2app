...
- S3 → notify Lambda for log uploads (`s3-log-notify`) — **source is not in this repo** (no `scripts/lambda/` exists; earlier docs pointed at a ghost path). Its `PLATFORM_PREFIXES` understands `desktop/`, `mobile/` and the legacy `service-logs/` / `feedback-logs/` prefixes; `desktop-overleap/` / `auto-overleap/` (see `desktop/src-tauri/src/log_upload.rs`) still need adding on the AWS side.
