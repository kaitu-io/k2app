...
**The customer buys `Product=router`; the dedicated line is an implementation detail.** Two plans: `router-std-1y` (`HardwareSKU=redmi-ax6s`, hardware + first year, $399) and `router-svc-1y` (`HardwareSKU=""`, renewal / BYO, $299). Both carry a `PrivateNodePlanSpec` identical to `pn-dc-2t`; `pn-*` plans stay `is_active=false` and are never sold directly.
