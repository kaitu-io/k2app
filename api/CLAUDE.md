...
- **Order-time** (`api_order.go`): `CreateOrderRequest.Shipping` required for hardware plans (`HardwareSKU != ""`) on real (non-preview) orders. Line products (`private_node` | `router`) bypass the App `tier` gate (`validatePurchase`) and the active-subscription one-off-purchase gate; `region` validation applies to both line products.
