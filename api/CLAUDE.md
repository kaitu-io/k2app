...
- **EDM dual sender**: `logic_email_task.go` picks `edmSenderOverleap` (`mail.Config("edm_overleap")`) when `brand == BrandOverleap` **and** `viper.GetString("edm.overleap_from_email")` is non-empty; otherwise falls back to the kaitu sender. Both keys (`edm.overleap_from_email` gate + `edm_overleap.*` qtoolkit sender block) must be set together — drift between them means the wrong From-address ships. 7 system email templates (verification code, new-device login, web login confirm, device transfer, password login code, password changed, device kick) have branded English variants via `brandedEmailTemplate[T].For(brand)`; the rest are kaitu-only by design (their entry points are brand-gated or channel-locked) — full list in `email_templates_overleap.go`'s header comment.
- **System mail sender per brand** (`logic_email.go`): `systemSenderForBrand(b)` returns `mail.Config("mail_overleap")` only when `b == overleap` **and** `mail_overleap.send_from` is non-empty; otherwise `nil` = global `mail.Send` (kaitu always; overleap falls back fail-open with a one-shot `Warnf` until SES verifies overleap.io — registration codes must not stop). `sendSystemEmailAs(ctx, b, to, subject, body)` is the single plain-text entry; `emailTo(ctx, b, ...)` takes the brand explicitly, `emailToUser` resolves it via `brandOfUser`, `SendSystemEmail` (CLI) is pinned to kaitu. `MailSendWith(ctx, sender, msg)` checks `mail.dev_mode` **before** touching the sender, so both identities are inert in dev. Template choice and sender choice are independent knobs — `For(brand)` picks copy, `sendSystemEmailAs` picks From.
