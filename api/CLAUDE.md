...
- **Payment channel gate**: `Brand.Config().AllowsPayment(channel)` — kaitu allows `wordgate` + `apple_iap`; overleap allows none yet (Phase 6). Handlers reject a disallowed channel with **405001 `ErrorPaymentChannelUnavailable`** (`api_order.go`, `api_apple_iap.go`). Both `api_webhook.go`'s wordgate handler and `logic_apple_iap.go`'s `creditAppleTransaction` carry a brand-mismatch sentinel that logs `error` and refuses to credit if a payment lands for a user whose brand doesn't allow that channel — this is fail-loud by design (a persistent mismatch will retry-storm from the provider's webhook retries), not a transient condition to silence.
...
### Phase 6 seams — check before turning on an overleap payment channel

`Brand.Config().AllowsPayment(...)` is the only gate keeping overleap orders from flowing today (`brandRegistry[BrandOverleap].PaymentChannels` is empty). Flipping it on for a real channel (Stripe / a new Apple bundle / Play Billing) needs these fixed first — none of them are exercised while overleap has zero payment channels, so they're currently dormant, not verified-safe:

- **`planByAppleProductID`** (`logic_apple_iap.go:38`) has **no brand filter** (`tx.Where(&Plan{AppleProductID: productID})`) — same shape of bug as the `getPlanByPID` fix in this review (finding #1). An Apple product ID collision or reuse across brands would credit the wrong brand's plan. Needs the same `ScopeBrand` treatment before Apple IAP goes live for overleap.
- **Webhook fail-loud sentinel routing**: `api_webhook.go`'s wordgate handler and `logic_apple_iap.go`'s `creditAppleTransaction` both log-and-refuse on a brand/channel mismatch (see above) — but that's a silent `log.Errorf`, not an alert. Before a second brand's webhooks are live in production, wire this sentinel to an actual paging/Slack alert route, not just a log line nobody watches.
- **Overleap winback campaign codes**: `winbackCampaigns` (`worker_renewal_reminder.go`) currently has no overleap-scoped codes — `campaignVarsForBrand` will just return an empty vars map for overleap recipients (safe today, but silently no-op — verify this is still the intended behavior once overleap has real campaigns to send).
- **`SavingsText` is hardcoded Chinese** (`worker_renewal_reminder.go:333`: `"立减 %s 起，最高立减 %s"`) regardless of `brand`. Harmless while overleap has no active campaigns/winback codes to trigger it, but it will leak Chinese copy into an English winback email the moment overleap gets a campaign code in `winbackCampaigns` — needs an English variant gated on `brand` before that happens.
