...
- **Payment channel gate**: `Brand.Config().AllowsPayment(channel)` — kaitu allows `wordgate` + `apple_iap`; overleap allows none yet (Phase 6). Both `api_webhook.go`'s wordgate handler and `logic_apple_iap.go`'s `creditAppleTransaction` carry a brand-mismatch sentinel that logs `error` and refuses to credit if a payment lands for a user whose brand doesn't allow that channel — this is fail-loud by design (a persistent mismatch will retry-storm from the provider's webhook retries), not a transient condition to silence.
