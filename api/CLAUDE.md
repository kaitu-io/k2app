...
- **Order-time** (`api_order.go`): `CreateOrderRequest.Shipping` required for hardware plans (`HardwareSKU != ""`) on real (non-preview) orders. **One router per account** is enforced there too: a real hardware order is rejected with `ErrorInvalidOperation` if `userHasRouter` (`logic_router_order.go`) finds a non-`expired` fulfillment (after a sync, so a stale stage over a dead line doesn't block) or a renewable line (active/grace/suspended, same set as `findRenewablePrivateLine`, unlocked read); previews pass so the site can quote. **服务套餐仅续费，无路由器用户下单被拒**: a real service-plan order (`HardwareSKU == ""`) is rejected with `ErrorInvalidOperation` unless `userCanBuyRouterService` passes — `userHasRouter`, or any hardware fulfillment (`hardware_sku <> ''`, any stage; the deprovisioned-line hardware customer whose new row inherits hardware fields). A user whose only history is expired BYO rows is rejected; previews pass. Line products (`private_node` | `router`) bypass the App `tier` gate (`validatePurchase`) and the active-subscription one-off-purchase gate; `region` validation applies to both line products.
