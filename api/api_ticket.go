...
// viper 旧键（support.email）只服务 kaitu；overleap 恒走品牌注册表 BrandConfig.SupportEmail。
func configSupportEmail(b Brand) string {
	email := ""
	if b != BrandOverleap { // viper 旧键只服务 kaitu
		email = viper.GetString("support.email")
	}
	if email == "" {
		email = b.Config().SupportEmail
	}
	return email
}
...
func api_create_ticket(c *gin.Context) {
	...
	// Auto-generated tickets bypass email + Slack notifications to avoid noise.
	// They are still persisted to the DB below for admin visibility.
	if req.AutoGenerated {
		log.Infof(ctx, "api_create_ticket: auto-generated ticket, skipping email and slack notification")
	} else {
		if err := sendTicketEmail(ctx, ReqBrand(c), userEmail, req); err != nil {
...
func sendTicketEmail(ctx context.Context, b Brand, userEmail string, req CreateTicketRequest) error {
	supportEmail := configSupportEmail(b)
