...
type Order struct {
	...
	// *string（非 default:null + string 零值）：wordgate 建单成功后 tx.Save(order) 会
	// Selects("*") 强制把全部列写进 UPDATE SET，此时 `default` 标签不再生效（那只管 Create
	// 路径省列）——string 零值 "" 仍会被写进 UPDATE，撞上 MariaDB/MySQL 对 type:json 列的
	// json_valid CHECK。nil 指针在 Create 与 Save 两条路径都序列化成 SQL NULL，二者都合法。
