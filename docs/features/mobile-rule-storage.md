...
- PR1: Desktop 和 Mobile 使用同一个 Engine 组装隧道，消除 80% 代码重复
- PR2: Desktop daemon 变为 Engine 的薄 HTTP 壳，不再自行组装隧道
- PR3: 用户可在移动端切换路由规则模式：global（全局代理）和 smart（智能分流）
- PR4: 规则模式切换需断开重连，不支持热切换
- PR5: 规则模式选择持久化到本地原生存储，重启后保留
- PR6: Go Engine 拥有可写存储目录，k2rule 可下载 GeoIP/域名规则到本地缓存
- PR7: iOS 双进程（App + NE）共享同一存储目录（App Group container）
- PR8: smart 模式下首次连接自动下载规则文件（k2rule lazy load），不阻塞连接建立
- PR9: webapp `connect(wireUrl)` 接口签名不变，K2Plugin 内部追加 rule 参数

## Technical Decisions

### TD1: 新建 `k2/engine/` 共享包
...
### TD2: `k2/mobile/mobile.go` 变为 gomobile 薄壳
...
### TD3: `k2/daemon/` 直接使用 `engine.Engine`
...
### TD5: `rule=smart|global` 由 K2Plugin 原生层追加到 URL

webapp `connect(wireUrl)` 签名不变。K2Plugin 在发起连接前读取本地存储的
ruleMode，追加 `&rule=xxx` 到 wireUrl。

### TD6: Engine 从 URL 或 Config 获取 RuleMode
...
- AC1: 新建 `k2/engine/` 包，包含 Engine struct + Config struct + Start/Stop/StatusJSON
- AC2: Engine.Start() 统一实现隧道组装流程：k2rule → transport → router → provider → tunnel
- AC3: fd >= 0 时使用平台 TUN fd + DNS middleware 拦截模式
- AC4: fd == -1 时自建 TUN（desktop）+ route exclusion 模式
- AC5: Config.DirectDialer != nil 时绑定到 transport
- AC6: Config.PreferIPv6 == true 时检查并替换 wireCfg.Host
- AC7: Config.Mode == "proxy" 时使用 ProxyProvider

### Desktop 迁移

- AC8: `daemon/tunnel.go` 的 `BuildTunnel()` 删除，daemon.doUp() 改用 engine.Engine
- AC9: daemon 现有 HTTP API（up/down/status/version/get_config/ping）行为不变
- AC10: daemon 自动重连、状态持久化、进程监控逻辑不变
- AC11: `cargo test` (desktop Rust) + `yarn test` (webapp) 通过

### Mobile 迁移

- AC12: `mobile/mobile.go` 变为 `engine.Engine` 的 gomobile 薄壳
- AC13: `Engine.Start(url, fd, dataDir)` — gomobile 导出三个基础类型参数

### Rule Mode

- AC14: Engine 从 URL query 解析 `rule=smart|global`，默认 `global`
- AC15: `rule=smart` → k2rule `IsGlobal: false` + CacheDir；`rule=global` → `IsGlobal: true`
- AC16: K2Plugin.swift 新增 `setRuleMode`，connect() 追加 `&rule=xxx`
- AC17: K2Plugin.kt 新增 `setRuleMode`，startVpnService() 追加 `&rule=xxx`
- AC18: PacketTunnelProvider 传 App Group `/k2` 路径作为 dataDir
- AC19: K2VpnService 传 `filesDir` 作为 dataDir
- AC20: definitions.ts + native-client.ts 新增 setRuleMode 声明和实现
- AC21: Dashboard UI 调用 setRuleMode 后断开重连生效

## Testing Strategy

- **engine 包单元测试**: Config 各组合（mobile/desktop/proxy）的组装逻辑
- **daemon 回归测试**: api_test.go 验证 up/down/status 行为不变
- **mobile 集成测试**: 手动验证 iOS/Android setRuleMode + connect + dataDir
- **iOS NE 冷启动**: 杀进程后系统自动重连，验证 rule 参数正确传递
- **Desktop 端到端**: `make dev` 验证连接/断开/重连/自动重连全流程

## Deployment & CI/CD

- gomobile bind 重新构建 AAR/xcframework
- CI 需确保 `go test ./engine/...` 通过
- 无 CI 流程变更，复用现有 pipeline
