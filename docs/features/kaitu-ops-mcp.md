...
- 用 MCP Server + Skill 替代 Web Dashboard 的批量脚本系统，实现 AI 驱动的服务器运维 (v1)
- MCP Server 提供原子能力（SSH 直连 + API 查询），Skill 提供运维知识和安全护栏 (v1)
- 节点操作通过 SSH 直连（不绕 Center API），消除中间人延迟 (v1)
- Center API 仅用于节点发现（`X-Access-Key` 认证，复用已有 admin 机制） (v1)
- 现有 Node/Tunnel model 不做任何变动 (v1)
- Cloud 相关工具延后实现（见 `docs/todos/kaitu-ops-mcp-cloud-tools.md`） (v1)

### MCP 工具集（2 个工具）

1. **`list_nodes`** — 从 Center API 获取所有节点列表，包含关联隧道信息 (v1)
   - 可选过滤：country, name
   - 返回：`[{name, ipv4, ipv6, country, region, tunnels: [{domain, protocol, port, serverUrl}]}]`
   - 数据源：`GET /app/nodes/batch-matrix`（MCP 侧过滤，只提取必要字段，丢弃批量脚本矩阵数据）(scrum-decided)
   - 认证：`X-Access-Key` header → Center API `handleAccessKeyAuth()` → `AdminRequired()`

2. **`exec_on_node`** — SSH 直连节点执行任意命令 (v1)
   - 输入：ip, command, timeout (default 30s)
   - 返回：`{stdout, stderr, exitCode, truncated}`
   - stdout 超过 10000 字符时截断，`truncated=true`
   - **stdout redaction**：返回前自动过滤敏感模式（`K2_NODE_SECRET=*`、`SECRET=`、64 位 hex 字符串等），替换为 `[REDACTED]` (scrum-decided)
   - **脚本传输**：通过 ssh2 exec channel 的 stdin pipe 传输文件内容，无需 shell 转义。MCP 实现读取本地文件 → pipe 到远程 `cat > /tmp/script.sh && bash /tmp/script.sh` (scrum-decided)
   - 覆盖所有节点操作场景：健康检查、日志查看、服务重启、文件操作、配置更新
   - ~~sudo 参数~~ — 删除。SSH 统一用 root 用户，sudo 无意义 (scrum-decided)

### Skill 文件（运维安全护栏）

Skill 文件 `.claude/skills/kaitu-node-ops.md` 编码节点基础设施的领域知识，作为 Claude 使用 MCP 工具时的运维安全护栏。

**定位**：Skill 是防误操作的最佳实践指导，不是安全边界。admin 本身拥有完全的 SSH root 权限。Skill 防止 AI 在运维过程中无意执行危险操作（如意外打印密钥、`docker compose down` 导致中断）。真正的安全由 MCP 层的 stdout redaction 提供技术保证。(scrum-decided)

#### 架构识别（双版本兼容）

线上同时存在两种架构的节点，Skill 必须教 Claude 先识别再操作 (scrum-decided)：
...
- `.env` 文件在 `/apps/kaitu-slave/.env`，核心变量 (v1)：
...
Skill 编码以下最佳实践规则。这些是防误操作的护栏，不是技术强制的安全边界 (scrum-decided)：
...
Skill 记录两种脚本执行方式 (scrum-decided)：
...
Skill 引用 `docker/scripts/` 下的运维脚本 (v1)：

| 脚本 | 用途 | 注意 |
|------|------|------|
| `prepare-docker-compose.sh` | 初始化节点部署目录 + 写入 docker-compose.yml | **旧版架构专用**。需要 sudo，已有部署的节点不要运行 (scrum-decided) |
...
- MCP 协议层（stdio）不需要认证 — 本地进程 (v1)
- Center API 认证：`X-Access-Key` header（复用已有 `handleAccessKeyAuth()`，User 表 `AccessKey` 字段，需 `IsAdmin=true`） (v1)
- SSH 认证：使用当前用户默认密钥（`~/.ssh/id_rsa` 或 `~/.ssh/id_ed25519`），可通过环境变量 `KAITU_SSH_KEY` 或配置文件覆盖 (scrum-decided)
- 配置文件路径：`~/.kaitu-ops/config.toml`（可选，环境变量优先） (v1)
...
- MCP Server 代码位于项目内 `tools/kaitu-ops-mcp/` (v1)
- Skill 文件位于 `.claude/skills/kaitu-node-ops.md` (v1)
- 脚本文件位于 `docker/scripts/` (v1)
- Claude Code MCP 配置在项目级 `.claude/settings.json` (v1)

## Technical Decisions

### 1. 实现语言：TypeScript

**决策**: 使用 TypeScript + `@modelcontextprotocol/sdk` (v1)
...
**决策**: MCP 提供原子工具 + stdout redaction（技术安全层），Skill 提供领域知识 + 运维护栏（行为指导层） (v1, scrum-decided)
...
**决策**: 节点操作 SSH 直连，使用当前用户默认 SSH 密钥或环境变量指定 (v1, scrum-decided)
...
**决策**: stdout 默认截断到 10000 字符 + 自动 redact 敏感模式 (v1, scrum-decided)
...
**决策**: 每次 tool call 创建新连接，不做连接池 (v1)
...
**决策**: 大脚本通过 ssh2 exec channel 的 stdin pipe 传输，不用 shell heredoc (scrum-decided)

**原因**:
- heredoc 需要处理 shell 转义（单引号、反引号、`$` 变量），AI 生成容易出 bug
- ssh2 的 stdin pipe 是 binary-safe 的，不经过 shell 解析
- 实现：MCP 读取本地文件 → pipe 到 `cat > /tmp/script.sh && chmod +x /tmp/script.sh && bash /tmp/script.sh`

### 8. 项目结构

**决策**: MCP Server 在 `tools/kaitu-ops-mcp/`，Skill 在 `.claude/skills/`，脚本在 `docker/scripts/` (v1)
...
docker/scripts/           # 运维脚本（从 ~/Downloads/scripts/ 整理）
...
- AC1: `list_nodes` 通过 `X-Access-Key` 从 Center API 获取节点列表，MCP 侧过滤只返回必要字段（name, ip, country, region, tunnels），可按 country/name 过滤 (v1, scrum-decided)
- AC2: `exec_on_node` 通过 SSH 直连节点执行命令，返回 stdout/stderr/exitCode (v1)
- AC3: `exec_on_node` stdout 超过 10000 字符时截断并标记 `truncated=true` (v1)
- AC4: `exec_on_node` stdout 返回前自动 redact 敏感模式（K2_NODE_SECRET 等） (scrum-decided)
- AC5: `exec_on_node` 支持自定义 timeout (v1)
- AC6: SSH 默认使用当前用户密钥，支持 `KAITU_SSH_KEY` 环境变量和配置文件覆盖 (scrum-decided)
- AC7: Center API 使用 config 或 `KAITU_ACCESS_KEY` 环境变量的 access_key 通过 `X-Access-Key` header 认证 (v1)
- AC8: 配置缺失时（无 config 文件且无环境变量），MCP Server 启动报告清晰的配置缺失错误 (v1)
- AC9: SSH 连接失败时返回明确的错误信息（IP 不可达、认证失败等） (v1)
- AC10: MCP Server 可被 Claude Code 通过 stdio 正常发现和调用 (v1)

### Skill 文件

- AC11: Skill 包含架构识别流程（`docker ps` 判断新版 k2v5 / 旧版 k2-slave） (scrum-decided)
- AC12: Skill 主体描述新版 k2v5 架构（4 容器、依赖链、网络模式），附带旧版 k2-slave 兼容映射 (scrum-decided)
- AC13: Skill 包含 `.env` 所有核心变量的说明 (v1)
- AC14: Skill 包含标准运维操作命令表（容器名按架构版本替换） (v1)
- AC15: Skill 包含 7 条运维安全护栏，明确定位为防误操作最佳实践（非安全边界） (scrum-decided)
- AC16: Skill 记录两种脚本执行模式（小命令直接 exec / 大脚本 stdin pipe） (scrum-decided)
- AC17: Skill 引用 `docker/scripts/` 下的脚本及使用注意事项，标记 `prepare-docker-compose.sh` 为旧版专用 (scrum-decided)

### 脚本

- AC18: `~/Downloads/scripts/` 中的脚本整理到 `docker/scripts/`，修正文件名 (v1)

## Testing Strategy

- 单元测试：config 解析（TOML + env fallback）、API response 字段过滤、stdout 截断、stdout redaction (v1)
- 集成测试：mock SSH server 验证 exec 行为和 stdin pipe 脚本传输 (v1)
- 手动测试：安装到本地 Claude Code，验证 2 个 MCP 工具均可正常调用 (v1)
- 不需要 CI — 这是私有运维工具 (v1)

## Deployment & CI/CD

- 构建：`cd tools/kaitu-ops-mcp && npm install && npm run build` (v1)
- Claude Code 配置：项目级 `.claude/settings.json` 注册 MCP server (v1)
- 用户配置：设置 `KAITU_ACCESS_KEY` 和 `KAITU_SSH_KEY` 环境变量，或创建 `~/.kaitu-ops/config.toml` (v1)
- 无 CI/CD — 开发者手动构建 (v1)
