...
只需处理 `x.y.z` 格式（忽略 `-beta.1` 后缀），因为公告版本过滤不需要 pre-release 精度。
...
逻辑：
1. 查询 `WHERE is_active = true AND (expires_at = 0 OR expires_at > NOW())`
...
**`clientVersion` 来源**：从 `X-K2-Client` header 解析（已有 `parseClientHeader()` → `AppInfo.Version`）。Web 端无此 header，返回全部。
...
| 客户端版本 | API 响应 | 行为 |
|-----------|---------|------|
| v0.4.1 及以下 | 忽略 `announcements`，`announcement` 为 nil（无后端数据） | 无公告 |
