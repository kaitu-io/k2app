...
| A04 | D2 / D7 | single-instance | 第二实例 exit 0（~200ms），首实例窗口置前 | **PASS**（macOS，2026-08-18）— 第二实例 474ms exit 0，首实例收到唤起回调并存活；**Windows 0.4.8 真机 PASS 2026-08-21**——发布版 k2app.exe：实例 A(pid10564)存活/计数1，实例 B(pid6596) HasExited exitCode=0、计数仍为 1，单实例锁在窗口创建前生效 |
...
| D13 | D7 | **appbypass 应用列表可见性**（Windows 三连修） | `717552be` F2/F3/F4 | **PASS（发布码真机验证）** 2026-08-21 — 真 Windows 机（desktop-de632ne，Win10）本地 cargo-xwin 交叉编译 `Kaitu_0.4.8_x64.exe`（Wordgate LLC/Certum 签名，时间戳吻合），静默覆盖装 0.4.3→0.4.8（daemon 报 `{version:0.4.8,commit:6bc70b0}`，k2app.exe FileVersion 0.4.8，快捷方式指向 k2app.exe）。跑**发布版同源** `windows::enumerate()`（一次性 `--dump-installed-apps` 子命令，即改即还 main.rs、构建后 `git checkout` 字节还原）对真实注册表 → 返回 8 个 user-facing app。**F2 双视图证实**：Apple Software Update、Microsoft Edge 在注册表**仅存于 32 位 WOW6432 视图**（Program Files (x86)），二者均出现在输出中——旧「仅 64 位」码必漏。**F3 空 InstallLocation 兜底证实**：VC++ 2017 Redist 注册表 `InstallLocation=<EMPTY>`，输出 `id=C:\ProgramData\Package Cache\{80586c77...}` 系从 `DisplayIcon`（VC_redist.x64.exe,0）剥出目录——旧码会静默丢弃空条目。前提层（64/32 视图 17/8 分裂、空 InstallLocation）先前亦已确认。⚠️注：裸 cargo exe 缺 VCRUNTIME140_1.dll（该机未装、发布 bundle 不需），测试时旁置该 DLL 解决，不影响 enumerate 逻辑 |
