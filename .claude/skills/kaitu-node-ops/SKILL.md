...
**Canonical deployment = the single-compose `k2s` + `k2-sidecar` stack** (`docker/docker-compose.yml`, dir `/apps/kaitu-slave/`). Everything below assumes it. The old `k2-slave` SNI router is **legacy** — see §1.
...
6. **Never delete `/apps/kaitu-slave/`** — it is the entire node deployment.
...
**Canonical stack** (dir `/apps/kaitu-slave/`, images from `public.ecr.aws/d6n9t2r2/`):
...
## §2 Env Master Reference (`/apps/kaitu-slave/.env`)
...
| View `.env` (redacted) | `sed -E "s/(SECRET\|CLAIM\|TOKEN)=.*/\1=<redacted>/" /apps/kaitu-slave/.env` |
...
Two modes, first match wins: (1) `/apps/kaitu-slave/users` (bind-mounted), (2) Center remote `/slave/device-check-auth` (fallback). **Empty file = pure remote auth = default.** No restart needed (changes apply on next full auth; 1h tickets stay valid). Format = one `udid:token` per line.

| Operation | Command |
|-----------|---------|
| View | `cat /apps/kaitu-slave/users` |
| Add | `echo "udid:token" >> /apps/kaitu-slave/users` |
| Remove | `sed -i "/^UDID:/d" /apps/kaitu-slave/users` |
| Clear (remote only) | `truncate -s 0 /apps/kaitu-slave/users` |
...
exec_on_node(ip, "cd /apps/kaitu-slave && sudo docker compose pull && sudo docker compose up -d --remove-orphans")
```

- **`docker compose up -d` is the only way to apply `.env` changes** (`docker restart` reuses old env).
- If sidecar & k2s need different tags: `sudo sed -i "s#k2s:\${K2_VERSION:-latest}#k2s:v0.4.6-<sha>#" /apps/kaitu-slave/docker-compose.yml`.
...
| 1 | Containers up | `cd /apps/kaitu-slave && docker compose ps` | all Up, sidecar (healthy), no `k2-oc` orphan |
...
cd /apps/kaitu-slave && sudo docker compose up -d --remove-orphans
```
