...
| **[Auto] bad connection experience** | Ticket body already names `Server`, `Duration`, `Rule`. Grep the last `DIAG: session-end` and the 20 events before it. | `Duration=0s` or `<60s` = couldn't sustain. Check if last event was `wire-error`, `transport-race-fail`, or `transport-rerace`. |
