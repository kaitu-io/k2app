...
- `.github/workflows/build-mobile.yml`: a first `plan` job derives brands + platforms from the ref (mirrors `release-desktop.yml`): bare `v*` = kaitu on both platforms (+overleap when repo variable `OVERLEAP_MOBILE_CI == 'true'`); `v*-ios` / `v*-android` = kaitu, one platform; `v*-overleap-ios` / `v*-overleap-android` / `v*-overleap-mobile` = overleap only (never rebuilds or re-uploads kaitu); any other suffix → empty lists, every job skips. `workflow_dispatch` takes `platform` + `brand` (kaitu / overleap / both, default kaitu). `build-ios` / `build-android` read `fromJSON(needs.plan.outputs.<x>_brands)`. GitHub-hosted runners (`macos-26`, `ubuntu-latest`, ephemeral). ASC upload runs for **both** brands (same team + API key; ASC routes by the IPA's bundle id). Android: APK **and** AAB purity gates, `android-apk-<brand>` + `android-aab-<brand>` artifacts; the S3 upload is kaitu-only (overleap AAB goes to the Play Console by hand); `ANDROID_BUILD_REV` is passed like `IOS_BUILD_REV`. The purity gate is run against the brand-exact artifact path, never a glob (the desktop `.app.tar.gz` alphabetical-glob incident, `813bf3f5`). Success also triggers the `publish-web-ota` tail job for non-beta push tags (`-overleap-*` included — web OTA is always dual-brand).
- **Overleap legs are opt-in by tag**, not by a global switch: `OVERLEAP_MOBILE_CI` only adds overleap to bare tags and stays unset until the `OVERLEAP_ANDROID_STORE_PASSWORD` secret and the runner's `io.overleap` provisioning exist. `K2_APP_STORE_URL` (xcconfig), `OVERLEAP_APPSTORE_URL` (CI env), webapp `storeUrls.ios` and web `storeLinks.ios` are backfilled together in one commit once the App Store listing is live (spec 2026-09-06 §1.6); the Play URL is fixed by the package name and already filled in.
