...
- **storage_crypto.rs** — AES-256-GCM encryption for storage values. Key derived via HKDF-SHA256 from platform hardware ID (macOS: `kern.uuid`, Windows: SMBIOS UUID via PowerShell, Linux: `/etc/machine-id`). Encrypted values prefixed with `ENC1:`. Plaintext values (pre-encryption) read transparently for backward compatibility. Platform-specific hardware ID tests gated with `#[cfg(target_os)]`.
