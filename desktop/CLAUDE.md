...
- Windows NSIS 安装器的 LZMA 压缩载荷，和 macOS 二进制的 brotli 压缩同理，对 `strings` 门不可见
  （webapp payload 被压缩进安装包/二进制后，另一品牌的字符串肉眼看不出、`strings` 也抓不到）；
  真正的保障是打包前的 `webapp/scripts/check-brand-purity.sh` dist 门，加上品牌精确的产物收集路径
  （而非 `find *.app | head -1` 式模糊收集——见上面 `813bf3f5` 的教训）。
