...
- **Storage encryption**: Desktop `storage.json` values encrypted with AES-256-GCM. Key derived from platform hardware ID (macOS `kern.uuid`, Windows SMBIOS UUID, Linux `/etc/machine-id`) via HKDF-SHA256 — never stored. `ENC1:` prefix on encrypted values; plaintext values read as-is (backward compat). MCP Go reimplements same crypto with shared test vectors for session sharing (read-only).
