...
- **k2subs** — Subscription URL scheme (`k2subs://udid:token@host/api/subs`). Resolves to a list of `k2v5://` tunnels via `/api/subs`. Desktop daemon resolves server-side (persistent `Subscription` object with refresh loop + Phase-B outbound hot-swap). **Mobile has no appext-side resolver** (NE 50MB budget) — webapp resolves to a single `k2v5://` before `_k2.run('up')` via `webapp/src/services/subs-resolver.ts` (commit `e210564`). If raw `k2subs://` reaches mobile engine → code 570 "no k2v5 outbound configured"; that's always a webapp bug. Implication: mobile smart mode is "one-shot pick + retry on initial failure", not continuous optimization.
