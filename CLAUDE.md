...
- **Web OTA 是「谁最后发谁赢」**：`{brand}/web/latest.json` 是全局可变指针，版本第 4 段是**时间基**——旧 commit 的产物也永远"更新"，一定赢。**任何 `v*` tag 都会经 `build-mobile.yml` 的尾巴发双品牌 stable web OTA**（`release-desktop.yml` 的同名 job 另有 `should_run` 门把未知后缀挡掉，**两者不对称，只读一个必然误判**）。`publish-web-ota.yml` 的 `precheck` / `publish` 两道门拿线上出处（`{brand}/web/latest-source.json`，客户端不读）做祖先比对：联动路径遇到"比线上旧"会**跳过并发 Slack**（不把 app 发版搞红），显式路径直接**红**；要往回发必须 dispatch 时勾 `allow_rollback`。决策表与穷举测试在 `scripts/ci/web-ota-gate.mjs` / `.test.mjs`，设计在 spec §6.1。**webapp 的改动不发一次 OTA 就到不了任何已装客户端**——push main 什么都不发。
