...
- **`pre-build` also overwrites `k2/rule/embed/krs.tar.gz`** (tracked in the submodule as a 66 KB placeholder) with the ~2 MB CDN rule archive — never commit it. Offline, a dev build silently embeds the placeholder; only `EMBED_REQUIRE_FULL=1` (release CI) fails instead. CI `test-gateway-uat` runs `scripts/check-embed-size.sh`, which fails if the committed k2 HEAD blob exceeds 300 KB.
...
- **Artifact naming**: Desktop `Kaitu_{VERSION}_{ARCH}.{EXT}` (underscores); mobile uses the `kaitu/android/` CDN layout; overleap desktop builds are `Overleap_{VERSION}_{ARCH}.{EXT}` under CDN `/overleap/desktop/`; overleap mobile builds are `Overleap-{VERSION}.apk` / `Overleap-{VERSION}.aab` (no underscore — Android flavor naming) — **overleap Android is Play-only**: nothing is published under CDN `overleap/android/`, the AAB is a workflow artifact for the Play Console (kaitu also emits a `.aab`, unused). Release tags (desktop + mobile share one table, `scripts/ci/release-plan.mjs`): bare `v*` = **both brands on every platform they ship on**; `-kaitu` / `-overleap` = that brand everywhere; `-desktop` / `-mobile` / `-macos` / `-windows` / `-linux` / `-ios` / `-android` = kaitu narrowed; `-overleap-desktop` / `-overleap-mobile` / `-overleap-{macos,windows,ios,android}` = overleap narrowed. A brand scope never means different platforms in the two workflows — edit the script, not the YAML (a test fails if a suffix table is re-inlined). Linux desktop is **kaitu-only** (`scripts/ci/upload-release.sh` refuses overleap; `/i/k2` 404s off-brand). Details in `desktop/CLAUDE.md` / `mobile/CLAUDE.md`.
...
- **Web OTA 只由专用 tag 触发（2026-09-18 起）**：`{brand}/web/latest.json` 是全局可变指针，版本第 4 段是**时间基**——旧 commit 的产物也永远"更新"，一定赢，所以发布必须显式。触发只有一条自动路径：**推 `webapp/x.y.z[-brand]` tag**（裸=**双品牌**，`-kaitu` / `-overleap` 后缀=单品牌，`webapp/*` glob 因 `*` 不跨 `/` 天然匹配后缀）。tag 里的 `x.y.z` 必须 == `package.json`（剥掉品牌后缀后比对，见 `scripts/ci/web-ota-plan.mjs`）。**`v*` app 发版 tag 不再连带发 web OTA**——旧的 `build-mobile.yml` / `release-desktop.yml` 联动尾巴已删，两 workflow 的门不对称那个坑随之消失。副作用:发 app **不再**自动把 CDN web 追平到该 commit,想更新线上 web 必须主动推 `webapp/*` tag。kaitu / overleap **各自独立发布**（gate 只读在发的品牌的 provenance）。`publish-web-ota.yml` 的 `precheck` / `publish` 两道门拿线上出处（`{brand}/web/latest-source.json`，客户端不读）做**逐品牌**祖先比对：会倒退/岔开就直接**红**拒发，要往回发必须 dispatch 时勾 `allow_rollback`（gate 已收敛成 explicit-only,没有联动的 skip 分支了）。决策表与穷举测试在 `scripts/ci/web-ota-gate.mjs` / `.test.mjs`，tag→品牌解析在 `web-ota-plan.mjs` / `.test.mjs`，设计在 spec §6.1。**webapp 的改动不发一次 `webapp/*` tag 就到不了任何已装客户端**——push main 什么都不发。
