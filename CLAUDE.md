...
- **k2subs** — Subscription URL scheme (`k2subs://udid:token@host/api/subs`). Resolves to a list of `k2v5://` tunnels via `/api/subs`. **Desktop daemon only** — persistent `Subscription` object with refresh loop + Phase-B outbound hot-swap + probe-driven `ScoreSource` (`k2/config/subscription.go Pick()`). **Mobile is manual-only** — webapp passes a single `k2v5://` URL to `_k2.run('up')`; appext never sees `k2subs`. Rationale: iOS NE has a 50MB jetsam limit and webapp-side resolution during VPN-up causes double-encapsulation (fetches go through the tunnel we're establishing). Users get data-driven manual picks via `probe.store` + `ProbeChip` on the tunnel list. If raw `k2subs://` reaches mobile engine → code 570 "no k2v5 outbound configured"; that's always a webapp bug.
- **probe.Registry** — In-memory per-URL measurement cache (`k2/probe/`). Records `wire.ProbeStats` from `wire.ProbeURL` (QUIC handshake + 8 echo rounds × 50ms). Used by: (a) daemon's background `probe.Service` loop (5 min cadence, uses active session's `DirectDialer` to bypass TUN); (b) `/api/core probe` action for ad-hoc measurements; (c) `Subscription.Pick` via `ScoreSource` interface to steer server selection. Flake tolerance: first `score==0` probe returns `ok=false` (no-data, neutral weight); two consecutive zeros confirm hard-exclude. TTL 15 min.
- **recommendScore** — Canonical `[0.0, 1.0]` tunnel recommendation signal (higher = better). Computed by the single-authority helper `api.ComputeRecommendScore(instance)` (commit `9e12d0b`). Emitted on both `/api/tunnels` (Dashboard `RecommendDot` UI) and `/api/subs` (daemon `Subscription.Pick` + webapp `pickWeighted`). Non-cloud nodes default to `0.5` neutral (not 0 — zero would blacklist). Legacy `weight` field still dual-emitted as `round(score*100)` for backward compat, remove one release after full rollout.
