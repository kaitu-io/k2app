...
- **K2Plugin dist/ must be committed**: `mobile/plugins/k2-plugin/dist/` is whitelisted in `.gitignore` and committed to git. Webapp depends on these type declarations for `tsc`. After editing any file in `mobile/plugins/k2-plugin/src/`, run `cd mobile/plugins/k2-plugin && npm run build` to regenerate dist/, then commit the updated dist/ files. The Makefile `build-webapp` target also runs `build-k2-plugin` as a safety net.
