...
- **Build-time debug switch**: `grep BUILD_DEBUG_SWITCH` finds all 3 locations: Go (`config.BuildDebugLogging`), Rust (`main.rs` level), TS (`config.store.ts` level). All set to debug for beta phase. Change to info/false for production.
